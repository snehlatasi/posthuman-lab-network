...
export default function AdminPeoplePage() {
  ...
  const loadPeople = async () => {
    setLoading(true);
    try {
      const res = await cmsApi.getPeopleAdmin();
      setPeople(res);
    } catch {
      // Fallback
    } finally {
        ...
        <div className="fixed inset-0 bg-carbon-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-carbon-900 border border-bone-50/15 p-6 rounded-3xl max-w-lg w-full space-y-4 shadow-2xl">
            ...
            <div className="space-y-3 text-xs">
              ...
              <input
                type="text"
                placeholder="Role (e.g. Lead Researcher, Author)"
              ...
              <input
                ...
                value={newPerson.affiliation}
                onChange={(e) => setNewPerson({ ...newPerson, affiliation: e.target.value })}
                className="w-full p-3 bg-carbon-950 border border-bone-50/15 rounded-xl text-bone-50"
              />
              <textarea
                rows={3}
                placeholder="Short Biography"
                value={newPerson.shortBio}
                onChange={(e) => setNewPerson({ ...newPerson, shortBio: e.target.value })}
