...
const mockPublicationsFallback: Record<string, PublicationApiDto> = {
  ...
  "lichen-synth": {
    ...
    content:
      "Lichen-Synth explores cellular automata and reaction-diffusion equations styled after symbiotic lichen growth patterns. By mapping algorithmic growth rules to real-time ambient soundscapes, this project decenters human aesthetic choices. The resulting visual topologies evolve organically, providing an interactive canvas that models nonhuman longevity and slow ecological time.",
...
export default function PublicationDetailPage() {
  ...
  const activePub = publication ||
    mockPublicationsFallback[slugStr] || {
      id: 0,
      title: slugStr.replace(/-/g, " ").toUpperCase(),
      slug: slugStr,
      summary: "Document details retrieved from Posthuman Lab Network repository.",
      content:
        "Detailed research content is synchronized with the Posthuman Lab Network knowledge base.",
    ...
    >
      <div className="space-y-8 font-sans">
        ...
        <div className="flex flex-wrap gap-4 text-xs font-mono text-carbon-900 font-bold border-b border-carbon-950/10 pb-4 mb-6">
          ...
          <span>•</span>
          <span className="text-earth-600 font-bold uppercase">
            Category: {activePub.publicationType}
          </span>
          {activePub.publishedAt && (
            <>
              <span>•</span>
              <span className="text-carbon-900 font-bold">
                📅 {new Date(activePub.publishedAt).toLocaleDateString()}
              </span>
            </>
          )}
        </div>
