    ...
    <div className="fixed inset-0 bg-carbon-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
      <div className="bg-carbon-900 border border-bone-50/15 p-6 sm:p-8 rounded-3xl max-w-3xl w-full space-y-6 shadow-2xl flex flex-col max-h-[85vh]">
        <div className="flex items-center justify-between border-b border-bone-50/10 pb-4 shrink-0">
          <div>
            <h3 className="font-serif text-xl font-bold text-bone-50 uppercase tracking-tight">Select Media Asset</h3>
            <p className="font-sans text-xs text-bone-200">Choose from existing Media Library or embed a YouTube video.</p>
          </div>
        ...
        <div className="flex space-x-3 shrink-0">
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-mono uppercase font-bold transition-all cursor-pointer ${
              activeTab === "library" ? "bg-earth-600 text-bone-50" : "bg-carbon-950 text-bone-200 hover:text-bone-50"
            }`}
          >
            Media Library
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-mono uppercase font-bold transition-all cursor-pointer ${
              activeTab === "youtube" ? "bg-earth-600 text-bone-50" : "bg-carbon-950 text-bone-200 hover:text-bone-50"
            }`}
          >
            Paste YouTube URL
          </button>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 pt-2">
              {filtered.map((a) => (
                ...
                >
                  ...
                  <span className="font-mono text-[10px] text-bone-200 truncate block">{a.title || a.filename}</span>
                </div>
              ))}
        ...
        {activeTab === "youtube" && (
          <form onSubmit={handleAddYouTube} className="space-y-4 flex-1">
            <div className="space-y-2">
              <label className="block text-xs font-mono text-bone-200 font-bold uppercase">YouTube Video URL</label>
              <input
                type="url"
                required
            ...
            <div className="space-y-2">
              <label className="block text-xs font-mono text-bone-200 font-bold uppercase">Video Title (Optional)</label>
              <input
                type="text"
