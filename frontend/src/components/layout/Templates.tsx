        ...
        <div className="py-16 md:py-24 relative">
          <Container className="grid grid-cols-12 gap-8 lg:gap-16">
                  ...
                  {sidebarLinks.map((link) => (
                    <Link
                      ...
                      className={`text-xs font-sans font-bold tracking-[0.12em] uppercase py-3 px-4 rounded-full transition-colors focus:outline-none ${
                        link.active
                          ? "bg-earth-500/15 text-earth-600 dark:text-earth-400 font-bold shadow-sm"
                          : "text-carbon-900 dark:text-bone-200 hover:bg-bone-50 dark:hover:bg-carbon-900 hover:text-earth-600 dark:hover:text-earth-400"
                      }`}
                    >
                      {link.label}
                    </Link>
            ...
            <article className="col-span-12 lg:col-span-8 space-y-12">
              <div className="editorial-surface rounded-[32px] p-8 md:p-12 border border-carbon-950/8 dark:border-bone-50/12 shadow-md">
                <Reveal className="prose max-w-none prose-headings:font-serif prose-headings:font-bold prose-headings:text-carbon-950 dark:prose-headings:text-bone-50 prose-p:font-sans prose-p:text-carbon-900 dark:prose-p:text-bone-200 prose-p:leading-relaxed prose-p:text-sm md:prose-p:text-base prose-a:text-earth-600 dark:prose-a:text-earth-400 hover:prose-a:text-earth-500 prose-strong:text-carbon-950 dark:prose-strong:text-bone-50 font-medium">
                  {children}
                </Reveal>
              </div>
                ...
                {filters.map((filter, idx) => (
                  <button
                    ...
                    className={`px-4 py-2 text-xs font-mono tracking-[0.16em] uppercase font-semibold rounded-full border transition-all focus:outline-none ${
                      filter.active
                        ? "bg-carbon-950 text-bone-50 border-carbon-950 dark:bg-earth-600 dark:border-earth-600"
                        : "bg-white/45 dark:bg-carbon-900/90 text-carbon-700 dark:text-bone-200 border-carbon-950/10 dark:border-bone-50/15 hover:border-earth-500/30 hover:text-earth-600 dark:hover:text-earth-400"
                    }`}
                  >
                    {filter.label}
                  </button>
