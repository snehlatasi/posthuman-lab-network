...
export const UpcomingEventsSection: React.FC = () => {
  return (
    <section id="events" className="py-24 md:py-32 border-t border-carbon-950/8 bg-gradient-to-b from-[#f9f3e9] to-[#f3eadb] relative">
        ...
        <Reveal staggerChildren={0.12} className="border-t border-carbon-950/10">
          {upcomingMeetingsList.map((event) => (
            <StaggerItem key={event.title}>
              <Link
                href={event.href}
                className="flex flex-col md:flex-row md:items-center justify-between py-6 px-6 my-3 rounded-2xl bg-white border border-carbon-950/10 shadow-sm hover:shadow-md hover:border-earth-600 transition-all duration-300 group focus:outline-none"
              >
                {/* Left: Date grid */}
                <div className="flex items-center space-x-6 md:space-x-10">
                  <div className="flex flex-col items-center justify-center p-3 w-16 h-16 rounded-xl bg-bone-50 border border-carbon-950/10 text-center group-hover:border-earth-600 transition-all duration-300">
                    <span className="font-mono text-2xl font-bold text-carbon-950 group-hover:text-earth-600 transition-colors leading-none">
                      {event.day}
                    </span>
                    <span className="font-mono text-[10px] text-carbon-900 uppercase font-bold tracking-widest mt-1">
                      {event.month}
                    </span>
                  </div>
                ...
                {/* Center: Location */}
                <div className="mt-4 md:mt-0 font-sans text-xs font-bold tracking-wider text-carbon-950 uppercase md:text-right pr-6">
                  📍 {event.location}
                </div>
