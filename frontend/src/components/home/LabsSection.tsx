...
export const LabsSection: React.FC = () => {
  return (
    <section id="labs" className="py-24 md:py-32 border-t border-carbon-950/8 bg-gradient-to-b from-[#f9f3e9] to-[#f3eadb] relative">
      <Container className="space-y-16">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 max-w-5xl">
          <div className="space-y-4">
            ...
            <p className="font-sans text-sm md:text-base text-carbon-800 leading-relaxed max-w-xl font-medium">
        ...
        <Reveal staggerChildren={0.15} className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
          {featuredLabsList.map((lab) => (
            <StaggerItem key={lab.name}>
              <ContentCard href={lab.href} className="border border-carbon-950/10 bg-white hover:bg-white hover:border-earth-600 h-full flex flex-col justify-between p-8 min-h-[300px] shadow-md hover:shadow-xl transition-all duration-300">
                <div className="space-y-6">
                  {/* Number & Category */}
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-xs text-earth-600 font-bold uppercase tracking-wider block">
                      {lab.tag}
                    </span>
                    <span className="font-mono text-lg font-bold text-carbon-950">
                      {lab.number}
                    </span>
                  </div>

                  <div className="space-y-2">
                    <h3 className="font-serif text-xl md:text-2xl font-bold text-carbon-950 group-hover:text-earth-600 transition-colors">
                      {lab.name}
                    </h3>
                    <p className="font-sans text-xs md:text-sm text-carbon-900 leading-relaxed font-semibold">
                      {lab.description}
                    </p>
                  </div>
                </div>
                ...
                {/* Card CTA arrow */}
                <div className="pt-6 border-t border-carbon-950/8 flex justify-between items-center text-xs font-sans font-bold tracking-wider text-carbon-950 group-hover:text-earth-600 transition-colors">
